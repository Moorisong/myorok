...
| 기간 | 주요 목적 | 표현 방식 (밀도) |
| :-- | :-- | :-- |
| **15일 (15d)** | 상세 확인 | **일 단위 (Daily)** <br> - Dot Chart (증상) <br> - Bar Chart (수액) |
| **1개월 (1m)** | 최근 추이 | **일 단위 (Daily)** <br> (15일과 동일, 표시 개수만 증가) |
| **3개월 (3m)** | 문제 구간 파악 | **주 단위 (Weekly Aggregation)** <br> - Bar Chart (주간 합계) |
| **6개월 (6m)** | 장기 패턴 요약 | **월 단위 (Monthly Aggregation)** <br> - 6개월 전용 월별 요약 차트 |
...
1. 기간별 표현 전략 (섹션 2) 확인
2. 데이터 타입별 차트 적용 (증상: Dot+Partial Line/Bar, 수액: Bar)
3. 전체 기간 선택 시 Summary Cards로 전환

#### 2. Custom Metrics Chart 작업 (`/charts/custom`)
1. 데이터 개수 기반 차트 타입 자동 선택 (섹션 10.1)
2. 기간별 집계 함수 적용 (`getMetricRecordsAggregated`)
3. UI/UX 개선 사항 적용 (줌/스크롤, Tooltip, 요약 카드)

#### 3. DB/서비스 함수 구현
1. `getMetricRecordsAggregated()` 함수 구현 (섹션 9.1)
2. 기존 `getMetricRecords()` 함수 개선 (섹션 9.2)
...
### 주의사항
-   **이벤트성 데이터는 0을 포함한 연속 라인 금지** (의료적 오해 방지, 단 연속 발생 시 부분 라인은 허용)
