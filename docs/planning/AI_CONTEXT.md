...
| 데이터 저장 | SQLite (expo-sqlite) | 로컬 저장 (Offline-first) |
| 서버 | Node.js + MongoDB | 백업, 복원, 인증 |
| 로그인 | 카카오 로그인 | 계정 연동 필수 |
| 사진/영상 | 없음 | MVP 제외 |
...
| 상태 저장 | `isPro` boolean 로컬 저장 + 서버 검증 |
| 서버 검증 | 있음 | 영수증 검증 및 상태 관리 |
